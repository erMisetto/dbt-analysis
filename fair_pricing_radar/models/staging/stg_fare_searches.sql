...
{% set geo_seed  = ref('seed_airport_geo') %}
{% set fare_seed = ref('seed_fare_bucket_map') %}
...
with raw as (
    ...
    from {{ source('raw_airfares', 'flight_pricing_data') }}
...
add_geo as (
    ...
    from prep p
    left join {{ geo_seed }} og on p.origin      = og.iata_code
    left join {{ geo_seed }} dg on p.destination = dg.iata_code
),
